...
(function($) {
    ...
    quail.components.label = function(quail, test, Case, options) {
        ...
        $scope.each(function() {
            ...
            $local.find(options.selector).each(function() {
                if (
                    (!$(this).parent('label').length || !$local.find('label[for=' + $(this).attr('id') + ']').length || !quail.containsReadableText($(this).parent('label'))) &&
    ...
    quail.strings.newWindow = [/new (browser )?(window|frame)/, /popup (window|frame)/];
    ...
    quail.strings.siteMap = ['site map', 'map', 'sitemap'];
        ...
        test.get('$scope')
            ...
            .each(function() {
                ...
                var expected = $(this)
                    ...
                    .data('expected');
                if (
                    quail.cleanString($(this).attr('alt')) ===
                    quail.cleanString(
                        $(this)
                            .parent('a')
                            .text()
    ...
    quail.doNotUseGraphicalSymbolToConveyInformation = function(quail, test, Case) {
        // Passes and fails.
        test.get('$scope')
            .find(quail.textSelector + ':not(abbr, acronym)')
            .each(function() {
                var whiteList = '✓';
                var blackList = '?xo[]()+-!*xX';
    ...
    quail.documentLangIsISO639Standard = function(quail, test, Case) {
        ...
        var _case = Case({
            element: $element[0],
            expected: $element.closest('.quail-test').length ? $element.closest('.quail-test').data('expected') : $element.data('expected')
        });
    ...
    quail.documentVisualListsAreMarkedUp = function(quail, test, Case) {
        var itemStarters = [
            ...
            '\\.\\s',
            'x\\s', // characters that should be followed by a space
            '&bull;',
            '&#8226;',
            '&gt;', // HTML entities
            '[0-9]+\\.',
            '\\(?[0-9]+\\)', // Numbers: 1., 13., 13), (14)
        ...
        var symbols = RegExp(
            ...
            '[\\s]*' + // Optionally followed by white space characters
                '(' +
                itemStarters.join('|') +
                ')', // Followed by a character that could indicate a list
            'gi'
        ); // global (for counting), case insensitive (capitalisation in elements / entities)
    ...
    quail.focusIndicatorVisible = function(quail, test, Case) {
        test.get('$scope')
            ...
            .each(function() {
                ...
                var boxShadow =
                    $(this).css('box-shadow') && $(this).css('box-shadow') !== 'none'
                        ? $(this)
                              .css('box-shadow')
                              .match(/(-?\d+px)|(rgb\(.+\))/g)
                        : false;
            ...
            .each(function() {
                var _case = Case({
                    ...
                    status:
                        $(this)
                            .text()
                            .replace(/^\s+|\s+$/gm, '').length > headerMaxLength
                            ? 'failed'
                            : 'passed'
                });
                test.add(_case);
        ...
        test.get('$scope')
            ...
            .each(function() {
                ...
                if (!$(this).attr('for')) {
                    ...
                    if (
                        !test.get('$scope').find('#' + $(this).attr('for')).length ||
                        !test
                            .get('$scope')
                            .find('#' + $(this).attr('for'))
                            .is(':input')
        ...
        test.get('$scope')
            ...
            .each(function() {
                ...
                if ($(this).next('a').length) {
                    var nextText = $(this)
                        .get(0)
                        .nextSibling.wholeText.replace(unreadableText, '');
            ...
            .each(function() {
                ...
                if (
                    $(this)
                        .parents('table')
                        .first()
                        .find('th#' + $(this).attr('axis')).length === 0
                ) {
                    _case.set({
                        status: 'failed'
                    });
    ...
    quail.tableHeaderLabelMustBeTerse = function(quail, test, Case) {
        test.get('$scope')
            ...
            .each(function() {
                ...
                test.add(_case);
                if ($(this).text().length > 20 && (!$(this).attr('abbr') || $(this).attr('abbr').length > 20)) {
                    _case.set({
                        status: 'failed'
                    });
                } else {
    ...
    quail.tableSummaryDoesNotDuplicateCaption = function(quail, test, Case) {
        test.get('$scope')
            ...
            .each(function() {
                if (
                    quail.cleanString($(this).attr('summary')) ===
                    quail.cleanString(
                        $(this)
                            .find('caption:first')
                            .text()
    ...
    quail.whiteSpaceInWord = function(quail, test, Case) {
        ...
        test.get('$scope')
            .find(quail.textSelector)
            .each(function() {
                nonWhitespace = $(this).text()
                    ? $(this)
                          .text()
                          .match(/[^\s\\]/g)
                    : false;
                whitespaceGroup = $(this).text()
                    ? $(this)
                          .text()
                          .match(/[^\s\\]\s[^\s\\]/g)
                    : false;
