    ...
    CKEDITOR.plugins.a11ychecker.quickFixes.get({
        ...
        callback(QuickFix) {
            ...
            AnchorsMerge.prototype.fix = function(formAttributes, callback) {
                const issueElement = this.issue.element;
                let nextSibling = issueElement.getNext();
                const initialHref = issueElement.getAttribute('href');
                let extraInnerHtml = '';
                const isWhitespace = function(node) {
                    return node.type === CKEDITOR.NODE_TEXT && node.getText().match(/^[\s]*$/);
                };
                const iterationAllowed = function(node) {
                    if (!node) {
                        return false;
                    }

                    // We only allow anchors that have the same href as the initial one
                    return (node.getName && node.getName() == 'a' && nextSibling.getAttribute('href') == initialHref) || isWhitespace(node); // Or whitespace text nodes.
                };
                let curNode;
