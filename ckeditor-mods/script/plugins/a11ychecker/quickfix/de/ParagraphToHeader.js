    ...
    CKEDITOR.plugins.a11ychecker.quickFixes.get({
        ...
        callback(ElementReplace) {
            ...
            ParagraphToHeader.prototype.display = function(form, editor) {
                const levelDict = this._getFormHeaderLeves(editor);
            ...
            ParagraphToHeader.prototype._getPreferredLevel = function(editor) {
                ...
                const headerTagRegExp = /^h[1-6]$/i;
                const range = new CKEDITOR.dom.range(editable.getDocument());
                let walker;
                let prevElement;
            ...
            ParagraphToHeader.prototype._removeBoldTag = function() {
                const isElementEvaluator = function(el) {
                    return el.type === CKEDITOR.NODE_ELEMENT;
                };
                const elem = this.issue.element;
                const innerElement = elem.getFirst(isElementEvaluator);
                // If first child element is at the same time last element child, then it means it has only this element.
                const hasSingleElement = innerElement && innerElement.equals(elem.getLast(isElementEvaluator));
                const suspiciousTagNames = ['strong', 'b'];
            ...
            ParagraphToHeader.prototype._getPossibleLevels = function(editor) {
                const tags = (editor.config.format_tags || '').split(';');
                const ret = {
                    min: 1,
                    max: 6
                };
            ...
            ParagraphToHeader.prototype._getFormHeaderLeves = function(editor) {
                const dict = {};
                const boundaries = this._getPossibleLevels(editor);
                const preferredLevel = this._getPreferredLevel(editor);
