    ...
    CKEDITOR.plugins.a11ychecker.quickFixes.get({
        ...
        callback(QuickFix) {
            ...
            DateUnfold.prototype.fix = function(formAttributes, callback) {
                ...
                let innerText = issueElement.getText();
                const that = this;

                innerText = innerText.replace(/(\d{1,2}[.\/-]\d{1,2}[.\/-]\d{2,4})/g, (occur) => {
                    const extractedDate = that.parseDate(occur);
            ...
            DateUnfold.prototype.getFriendlyDate = function(dateObj) {
                ...
                const monthName = monthNames.en[Number(dateObj.month - 1)];
