...
const wb_formats_2 = [['xlml', 'xlml', 'xls']];
...
if (program.req) { program.req.split(',').forEach((r) => {
...
function isfmt(m /*: string */) /*: boolean */ {
    ...
    let t = m.charAt(0) === '.' ? m : `.${  m}`;
    return program.output.slice(-t.length) === t;
}
workbook_formats.forEach((m) => {
    if (program[m[0]] || isfmt(m[0])) {
        wb_fmt();
    }
});
wb_formats_2.forEach((m) => {
    if (program[m[0]] || isfmt(m[0])) {
        wb_fmt();
    }
});
...
if (program.sparse) opts.dense = false;
else opts.dense = true;
...
function outit(o, fn) {
    if (fn) fs.writeFileSync(fn, o);
    else console.log(o);
}

function doit(cb) {
    /*:: if(!wb) throw new Error("unreachable"); */
    if (program.book) { wb.SheetNames.forEach((n, i) => {
		/*:: if(!wb) throw new Error("unreachable"); */
...
default:
        if (!program.book) {
            let stream = X.stream.to_csv(ws, { FS: program.fieldSep, RS: program.rowSep });
...
function dump_props(wb) {
    let propaoa = [];
    if (Object.assign && Object.entries) propaoa = Object.entries({ ...wb.Props, ...wb.Custprops });
    else {
        let Keys;
        var pi;
        if (wb.Props) {
